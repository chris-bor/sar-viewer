...
const float NaN = 2.139095040000000e+009; // NaN float
...
*/
const double QCPRange::minRange = MIN_RANGE;
...
*/
const double QCPRange::maxRange = MAX_RANGE;
...
void QCPAxis::setScaleRatioSlot(const QCPRange &range) {
//    qDebug() << __FILE__ << __FUNCTION__ << "()";
    if(QCPAxis *axis = qobject_cast<QCPAxis*>(sender())) {
        this->setScaleRatio(axis, 1.0);
    }
}
...
//    qDebug() << __FILE__ << __FUNCTION__ << "()";
  ...
  rescaleAxes();
  updateAxisRect();
  emit xAxis->rangeChanged(xAxis->range());
  replot();
}
...
void QCustomPlot::wheelEvent(QWheelEvent *event)
{
  ...
  if (mInteractions.testFlag(iRangeZoom))
  {
    if (mRangeZoom != 0)
    {
      ...
      xAxis->rangeChanged(xAxis->range());
      replot();
    }
  }
...
void QCustomPlot::xRangeChanged() {
    emit xAxis->rangeChanged(xAxis->range());
}
...
QCPCurve::QCPCurve(QCPAxis *keyAxis, QCPAxis *valueAxis) :
    QCPAbstractPlottable(keyAxis, valueAxis), NaNPointF(QPointF(NaN, NaN))
{
  ...
  mCurveType = QCPCurve::ctNoTypeSpecyfied;
...
void QCPCurve::setCurveType(QCPCurve::CurveType type)
{
    mCurveType = type;
}
...
//void QCPCurve::addData(double t1, double key1, double value1, double t2, double key2, double value2, uint nr_points)
//{
//    QCPCurveData newData1, newData2;
//    QCPCurveDataMap::const_iterator it;
//    // Less points than needed so insert normaly data( .t should be prepare earlier )
//    if(mData->size() != (nr_points*2)) {
//        newData1.t = t1;
//        newData1.key = key1;
//        newData1.value = value1;
//        newData2.t = t2;
//        newData2.key = key2;
//        newData2.value = value2;
//    }
//    else {
//        // usun
//        for(int i = 0; i < ((mData->size()/2)-1); i++){
//            (mData->constBegin()+i).key() = (mData->constBegin()+1+i).key();
//            (mData->constBegin()+i).key() = (mData->constBegin()+1+i).key();
//        }
//    }

//    if (!mData->isEmpty()) {// no empty
//        newData1.t = (mData->constEnd()-1).key()+1;
//    }
//    else {
//        newData1.t = t1;
//        newData2.t = t2;
...
void QCPCurve::draw(QCPPainter *painter)
{
  ...
  if (mLineStyle != lsNone && mainPen().style() != Qt::NoPen && mainPen().color().alpha() != 0)
  {
    ...
    if (mParentPlot->plottingHints().testFlag(QCP::phFastPolylines) &&
        painter->pen().style() == Qt::SolidLine &&
        !painter->pdfExportMode() && mCurveType == QCPCurve::ctMapCurve)
    {
        for (int i=0; i<lineData->size()-1; i++) {
            if( lineData->at(i) == NaNPointF || lineData->at(i+1) == NaNPointF) {
                continue;
            }
    ...
    {
        qDebug() << "*********drawPolyline(QPolygonF(*lineData));";
      painter->drawPolyline(QPolygonF(*lineData));
    }
  }
  // draw scatters:
...
//  bool firstPoint = true; // first point must always be drawn, to make sure fill works correctly
  ...
  {
      if(it.value().value == NaN){ // if NaN
          lineData->append(NaNPointF);
          continue;
      }
      // if no NaN
    x = it.value().key;
    ...
    /* Function modified specialy for SarViewer */
    if( currentRegion == 5 ) {
